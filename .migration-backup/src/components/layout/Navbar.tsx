    ...
    <>
      <header
        className={cn(
          ...
          scrolled
            ? "bg-black/90 backdrop-blur-xl border-b border-[#161616]"
            : "bg-transparent",
        )}
      >
          ...
          <div className="hidden md:flex items-center gap-1 flex-shrink-0">
            {SOCIAL_LINKS.map(({ icon: Icon, href, label }) => (
              <a
                ...
                className="w-8 h-8 flex items-center justify-center rounded-md text-neutral-500 hover:text-neutral-100 hover:bg-[#111] transition-all duration-150"
                aria-label={label}
              >
                <Icon size={15} />
              </a>
        ...
        <div className="absolute bottom-0 left-0 h-px w-full bg-[#161616]">
          <motion.div
            className="h-full bg-indigo-600/60"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed top-14 left-0 right-0 z-40 bg-black/95 backdrop-blur-xl border-b border-[#1e1e1e] md:hidden"
          >
            <div className="max-w-6xl mx-auto px-6 py-4 flex flex-col gap-1">
              {NAV_LINKS.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={(e) => scrollTo(link.href, e)}
                  className="px-3 py-2.5 text-sm text-neutral-400 hover:text-neutral-100 rounded-md hover:bg-[#111] transition-all"
                >
                  {link.label}
                </a>
              ))}
