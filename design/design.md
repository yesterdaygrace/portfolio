...
version: alpha
name: Vellum
description: An essay-pinned-to-a-wall presentation system — a single monochromatic field of slate charcoal blue (#2E3344) with warm ivory type (#F7F1E2) floating centered on it, every slide. Italic Cormorant Garamond carries every headline at all sizes — the italic serif is the personality, against the restrained colorfield. DM Sans handles body in a quiet supporting role. Courier Prime mono provides the typed annotation voice — appearing as a "pin-note" attribution sitting in the bottom-left corner of every slide. The mood is gallery exhibition wall meets archive folder — quiet, monochromatic, deeply still. One field, one material accent, zero motion.
...
colors:
  ...
  slate-mid: "#3B4051"
  ivory: "#F7F1E2"
  ivory-2: "rgba(247,241,226,0.84)"
  ivory-3: "rgba(247,241,226,0.68)"
  emphasis-terracotta: "#F3C8B4"
  terracotta: "#D8A08A"
  terracotta-deep: "#7E5246"
  border: "rgba(247,241,226,0.20)"
...
color-aliases:
  ...
  c-fg-3: ivory-3
  c-fg-light: ivory
  c-emphasis: emphasis-terracotta
  c-accent: terracotta
  c-accent-deep: terracotta-deep
  c-border: border
...
components:
  pin-annotation:
    ...
    color: "{colors.terracotta}"
    maxWidth: "22vw"
    description: "The system's signature element — a small stack of Courier Prime mono lines in terracotta, sitting in the bottom-left corner of every slide. Holds slide counter (e.g., '03 / 09'), a short pinned label, and an optional second pin note. Functions as the 'tag stuck to the wall.'"
  kicker:
    typography: "{typography.label}"
    color: "{colors.terracotta}"
    description: "Small Courier Prime mono label in terracotta, sits above a headline as a section marker."
  rule:
    width: 28px
    height: 1px
    background: "{colors.terracotta}"
    description: "A 28px hairline accent rule in terracotta, used as a small kicker separator."
  chrome-bar:
  ...
  bullet-list-numbered:
    ...
    description: "Numbered list using CSS counters — the counter renders in Courier Prime mono at label size in terracotta, with a 2em column for the number and 0.5em gap to the body."
  pin-stat:
  ...
  pin-stat-label:
    ...
    color: "{colors.ivory-2}"
    description: "Small mono caption beneath a pin-stat numeral."
  compare-panel-dark:
    background: "{colors.slate-deep}"
    description: "Left compare panel — a slightly deeper slate to create internal contrast against the standard slate field."
  compare-panel-light:
    background: "{colors.slate-mid}"
    borderLeft: "1px solid {colors.border}"
    description: "Right compare panel — a slightly lighter slate with a hairline left-border, creating the two-shade panel pair."
  bar-fill-default:
    background: "{colors.ivory-3}"
    description: "Default chart bar fill — ivory at 68% opacity (the quiet tier color)."
  bar-fill-accent:
    background: "{colors.ivory}"
    description: "Highlighted chart bar — full ivory."
  chart-baseline:
    height: 1px
    background: "{colors.border}"
    description: "1px hairline baseline beneath chart bars."
  img-placeholder:
    background: "rgba(46,51,68,0.12)"
    border: "1px dashed {colors.border}"
    description: "Image slot — translucent slate fill with a dashed ivory-low-opacity border, centered mono label inside."
  quote-mark:
    typography: "{typography.quote-mark}"
    color: "{colors.terracotta}"
    description: "A 7vw italic Cormorant Garamond opening quote glyph in terracotta, sitting centered above a centered pull-quote. Terracotta is the system's only large-graphic accent."
---

## Overview

Vellum is a **monochromatic essay-on-a-wall** presentation system. The visual premise is severe and tender at once: every slide is the same slate charcoal-blue field (`{colors.slate}` — #2E3344), with warm ivory type (`{colors.ivory}` — #F7F1E2) floating centered on it. There is no light/dark theme and no competing background family. The field is the constant; the type and a single small annotation in the bottom-left corner are everything else.

The typographic premise is **italic serif as personality**. Cormorant Garamond runs every headline at every scale — display through h3, quote-text, stat-value — in italic at weight 400. The italic serif at large scale against the slate field reads as gallery wall text: personal, considered, slightly intimate. DM Sans steps in for body and lead paragraphs in a quiet supporting role; the body is meant to recede so the italic serif headlines lead the eye. Courier Prime monospace carries the "pin-note" — a small stack of typed-attribution lines that sits in the bottom-left corner of every slide.

The color philosophy is **one field, warm material accent**. The field is `{colors.slate}`. Type is `{colors.ivory}` at full opacity for primary, 84% for secondary, and 68% for tertiary. Light terracotta (`{colors.emphasis-terracotta}`) marks headline emphasis, while accessible terracotta (`{colors.terracotta}`) carries kickers, pin-note text, the accent rule, and list counters. The deeper terracotta (`{colors.terracotta-deep}`) is reserved for filled states and panel details, so the requested muted terracotta remains visible without sacrificing text contrast.

Depth is **flat and centered**. There are no drop shadows. No rounded corners. No gradients. Borders are 1px hairlines at 20% ivory opacity, present mostly as faint structural marks rather than dividers. The compare layout is the single exception: it uses two slightly-different slate shades (`{colors.slate-deep}` and `{colors.slate-mid}`) to create a visible split panel, but even there, the difference is subtle. Animation is set to zero duration — slides do not transition. The system is, by design, completely still.

**Density philosophy: sparse and centered.** Vellum slides are pinned essays — each slide holds a small amount of content, centered in the canvas with significant breathing room above, below, and to the sides. A statement slide is one short headline centered on the field. A cover is a kicker, a display title, a lead sentence, and the pin-note in the corner — that's it. A list slide is four numbered rules in a 60% width column at center. The system reads as broken when crowded; the empty slate field around the content is structural, not negative space. It's wall.

**Key Characteristics:**
- Single monochromatic field — `{colors.slate}` (slate charcoal blue) — on every slide. No light theme, no inversion.
...
- Warm ivory type (`{colors.ivory}`) is primary; light terracotta (`{colors.emphasis-terracotta}`) is the only `<em>` color inside headlines.
- Terracotta (`{colors.terracotta}`) is the annotation accent — used for the quote-mark glyph, pin-note text, kickers, the 28px rule, and list counter markers.
- The deeper terracotta (`{colors.terracotta-deep}`) marks filled controls and the selected legacy-state tab.
- The pin-annotation (Courier Prime mono lines in terracotta in the bottom-left corner of every slide) is the system's signature element.
- Content is centered on every slide — text-align center on most layouts, with content occupying 55–80% of canvas width.
- Hairline 1px borders at 20% ivory opacity for chrome separators; otherwise no borders, no shadows, no elevation.
...
- **Slate** (`{colors.slate}` — #2E3344): Slate charcoal-blue field. The single surface across every slide, chosen to keep the portfolio serious without defaulting to pure black.
- **Slate Alt** (`{colors.slate-alt}` — #353A4A): A slightly lifted slate for adjacent-surface differentiation.
- **Slate Deep** (`{colors.slate-deep}` — #252A37): The darker compare-panel shade, used on the legacy side and dense content panels.
- **Slate Mid** (`{colors.slate-mid}` — #3B4051): The lighter compare-panel shade, used on the modernized side and selected content panels.
- **Ivory** (`{colors.ivory}` — #F7F1E2): Warm ivory primary text, selected for softer contrast against the slate field.
- **Ivory 2** (`{colors.ivory-2}` — rgba(247,241,226,0.84)): Secondary text for lead paragraphs and descriptions.
- **Ivory 3** (`{colors.ivory-3}` — rgba(247,241,226,0.68)): Tertiary text for quiet captions and chart labels.
- **Emphasis Terracotta** (`{colors.emphasis-terracotta}` — #F3C8B4): Light terracotta used for `<em>` emphasis inside italic headlines and selected headings.
- **Terracotta** (`{colors.terracotta}` — #D8A08A): Accessible terracotta annotation accent used for the quote mark, pin-note text, kicker labels, the 28px rule, and list markers.
- **Terracotta Deep** (`{colors.terracotta-deep}` — #7E5246): Muted terracotta used for filled states and panel borders, where it reads as material rather than small text.
- **Border** (`{colors.border}` — rgba(247,241,226,0.20)): Hairline border color, ivory at 20% opacity, used on chrome bars, stat dividers, and image placeholders.

### Defaults

- **Default surface**: `{colors.slate}` — every slide.
- **Default primary text color**: `{colors.ivory}`.
- **Default secondary text color**: `{colors.ivory-2}` (ivory at 84% opacity).
- **Default tertiary text color**: `{colors.ivory-3}` (ivory at 68% opacity).
- **Default emphasis color (inside headlines)**: `{colors.emphasis-terracotta}` — applied to `<em>` tags inside `display`, `h1`, and `h2`.
- **Default kicker / pin-note / accent rule color**: `{colors.terracotta}`.
- **Default large-graphic accent color**: `{colors.terracotta}` — used for the quote-mark glyph specifically.
- **Default filled-state color**: `{colors.terracotta-deep}`.
- **Default border**: `{colors.border}` — ivory at 20% opacity. Hairline only.

There is no semantic mapping between colors. Terracotta is not "warning," ivory is not "highlight." Ivory is the type; terracotta is the annotation voice; light terracotta is the emphasis flag. The palette is small enough that semantic roles stay deliberately restrained.
...
- **Default headline color**: `{colors.ivory}`.
...
- **An `<em>` tag inside an italic headline (`display`, `h1`, `h2`) renders as upright roman Cormorant Garamond at weight 600 in `{colors.emphasis-terracotta}`.** This is the system's emphasis mechanism — the inversion of italic to roman, plus the color shift, is non-negotiable. It is opposite the conventional "italic for emphasis" pattern; here, italic is the default and roman is the emphasis.
- **Every slide carries a pin-annotation in the bottom-left corner.** The pin-annotation is a stack of Courier Prime mono lines in `{colors.terracotta}` — typically a slide counter (e.g., "03 / 09") plus 1–2 short pin notes (label, attribution, or phrase). The pin-annotation is positioned absolute at `bottom: ~0.9 * pad-y`, `left: pad-x`, with max-width 22vw. A slide without a pin-annotation reads as a different system.
- **The pin-note typography uses Courier Prime at weight 500 in `{colors.terracotta}`** with letter-spacing 0.01em. Substituting any other face or color breaks the annotation register.
- **Kickers use Courier Prime mono at label size in `{colors.terracotta}`.** Sans kickers or italic kickers break the terracotta annotation convention.
- **The large quote-mark glyph (7vw italic Cormorant Garamond) renders in `{colors.terracotta}`, not ivory.** The terracotta quote mark is the only place a large-scale graphic is colored terracotta; elsewhere terracotta appears in small text.
- **Bullet-lists use a numbered counter rendered in Courier Prime mono at label size in `{colors.terracotta}`** — never bullet dots, never em-dashes. The numbered convention is the system's list signature.
...
- **Color shifts within the slate family** — `{colors.slate-deep}` and `{colors.slate-mid}` on compare panels create a subtle two-shade pairing.
- **Hairline borders at 20% ivory opacity** — visible but quiet structural marks.
- **Opacity tiers in ivory** — the three opacity tiers (full, 84%, 68%) provide enough text-color contrast to create hierarchy without introducing additional colors.
...
- **1px solid** in `{colors.border}` (ivory at 20% opacity) — the universal hairline. Used on chrome bars, foot bars, stat dividers, chart baselines, compare-panel left edges.
- **1px dashed** in `{colors.border}` — used on the image placeholder frame (the dashed border marks "this is a slot").
- **1px solid** at slightly higher opacity (rgba(247,241,226,0.18)) — used on the compare panel left/right divider.

There are no heavier borders in the system. Every line is 1px.

### Decorative Element Types

**Italic display headline** — Cormorant Garamond italic at weight 400, 4–11vw, centered on the slide in `{colors.ivory}`. The headline is the dominant visual element of every content slide.

**Pin-annotation** — a stack of 1–3 Courier Prime mono lines in `{colors.terracotta}`, positioned absolute at the bottom-left corner of every slide. Lines typically include a slide counter (e.g., "04 / 09"), a short typed label, and an optional second pin phrase. Lines stack with 0.3vh gap, max-width 22vw. This is the system's signature.

**Terracotta quote-mark glyph** — a 7vw italic Cormorant Garamond `"` in `{colors.terracotta}`, centered above a centered pull-quote. The only large-scale terracotta element in the system.

**Kicker** — Courier Prime mono at label size (0.72vw) in `{colors.terracotta}`, with 0.1em letter-spacing. Sits above a headline.

**28px accent rule** — a 28px × 1px horizontal bar in `{colors.terracotta}`. Used as a small kicker separator or chapter accent.

**Numbered list** — uses CSS `counter-reset: list-counter; counter-increment: list-counter` to render numbers in Courier Prime mono at label size in `{colors.terracotta}`, with a 2em column for the number and 0.5em gap to the body text (DM Sans lead).

**Pin-stat** — a centered column containing an italic serif stat numeral (5.5vw) above a mono caption in muted ivory. In a row, pin-stats are separated by single 1px hairlines at 20% ivory opacity; the last stat in row drops the border.

**Compare panel pair** — two side-by-side panels filling the slide. Left panel: `{colors.slate-deep}` background. Right panel: `{colors.slate-mid}` background with a 1px left-border. Each panel contains a small mono compare-label in `{colors.ivory-2}`, an italic h3, an italic lead, and a numbered list.

**Image placeholder** — a 1px-dashed rectangle in `{colors.border}` with a faint slate interior, centered Courier Prime mono label, minimum 28vh height. The dashed border is the "slot" indication.

**Chart bar** — a flat vertical rectangle in `{colors.ivory-3}` (default) or `{colors.ivory}` (accent). Bar values render in Courier Prime mono (`{typography.bar-val}`).

## Do's and Don'ts

### Do

- Fill every slide with `{colors.slate}` — the monochromatic field is the constant. There is no light alternate.
- Render every headline in italic Cormorant Garamond at weight 400, in `{colors.ivory}`. The italic serif at display scale is the system's identity.
- Place a pin-annotation in the bottom-left corner of every slide — Courier Prime mono in `{colors.terracotta}`, 1.15vw, max-width 22vw. The pin-annotation is the system's persistent signature.
- Use `<em>` inside headlines as upright roman (non-italic) at weight 600 in `{colors.emphasis-terracotta}`. This italic-to-roman emphasis pattern is non-negotiable.
- Color kickers, list counter markers, the 28px rule, and the large quote-mark glyph in `{colors.terracotta}`. Terracotta is the annotation accent and appears only in those specific contexts.
...
- Number bullet lists with Courier Prime mono counters in `{colors.terracotta}`. Bullet dots and em-dashes do not exist in this system.
- Leave generous empty slate field around every content block. Sparse is the correct register.
...
- The italic-to-roman `<em>` emphasis mechanism does not exist in CJK. Substitute with NSC 700 in `{colors.emphasis-terracotta}` for the equivalent "emphasis flag" effect.
...
- The terracotta quote-mark glyph (`"` at 7vw in `{colors.terracotta}`) does not exist in CJK conventions. Use `「` or `『` (full-width corner bracket) at the same 7vw size in terracotta — this is the correct Chinese equivalent and preserves the system's annotation accent signal.
...
1. Any new headline uses italic Cormorant Garamond at weight 400 in `{colors.ivory}`. Italic is the default, not the emphasis.
2. Any emphasized phrase inside a headline switches to upright roman at weight 600 in `{colors.emphasis-terracotta}`. The italic-to-roman inversion is the system's `<em>` mechanism.
3. Every new slide must include a pin-annotation in the bottom-left corner. The pin-annotation contains 1–3 lines of Courier Prime mono in `{colors.terracotta}`: typically a slide counter plus one or two short typed phrases.
4. Any new kicker, list counter marker, or 28px accent rule uses `{colors.terracotta}`. Terracotta is the annotation accent and is reserved for these specific elements.
5. Any new list is numbered with Courier Prime mono counters in terracotta. Dot and dash bullets do not exist in this system.
...
7. Any new background introduces a system break — there is only one surface color, `{colors.slate}`. Compare layouts use slight slate variants but do not introduce a different hue.
8. Any new font is forbidden. Cormorant Garamond, DM Sans, Courier Prime are the entire stack.
9. Any new chart, table, or data display uses 1px hairlines at 20% ivory opacity. No heavier borders, no shadows, no rounded chrome.
...
- The compare panel uses two near-identical slate shades (`{colors.slate-deep}` and `{colors.slate-mid}`) — at low-contrast displays or in heavy ambient light, the panel split may be hard to perceive.
- The terracotta accent color (`{colors.terracotta}` — #D8A08A) is intentionally softened and may read as a warm neutral rather than a bright signal on some displays.
